...
const emitUsersList = async (io) => {
  ...
  io.emit("usersList", users.map(usersController.toPublicUser));
};

const emitConversationToParticipants = (io, conversation) => {
  const conversationResponse =
    conversationsController.toConversationResponse(conversation);
...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    User.findByIdAndUpdate(
      ...
      .then((user) => {
        const publicUser = usersController.toPublicUser(user);
    ...
    socket.on("sendMessage", async (data) => {
      try {
        ...
        emitConversationToParticipants(io, updatedConversation);
